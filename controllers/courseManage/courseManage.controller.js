...
const courseManageController = {
  ...
  createCourse: (req, res) => {
    const { course_id, course_name, classroom, term_id, teachers, students } =
      req.body;
    ...
    connection.beginTransaction((err) => {
      ...
      connection.query(
        courseQuery,
        [course_id, course_name, classroom, term_id],
        (err, results) => {
          if (err) {
  ...
  getCourseById: async (req, res) => {
    ...
    try {
      ...
      const [courseResults] = await connection
        .promise()
        .query(courseQuery, [course_id]);
  ...
  updateCourse: async (req, res) => {
    ...
    try {
      ...
      const [updateResult] = await connection
        .promise()
        .query(updateCourseQuery, [course_name, classroom, term_id, course_id]);
      ...
      await connection
        .promise()
        .query(`DELETE FROM course_teachers WHERE course_id = ?`, [course_id]);
      ...
      await connection
        .promise()
        .query(`DELETE FROM course_members WHERE course_id = ?`, [course_id]);
  ...
  deleteCourse: async (req, res) => {
    ...
    try {
      ...
      const [courseResult] = await connection
        .promise()
        .query(checkCourseQuery, [course_id]);
  ...
  getCoursesInTermOfTeacher: (req, res) => {
    ...
    connection.query(sql, [teacher_id, term_id], (err, data) => {
      if (err) {
        ...
        return res
          .status(500)
          .send("Error executing query getCoursesInTermOfTeacher");
      }
  ...
  deleteMemberFromCourse: (req, res) => {
    ...
    if (role === "student") {
      ...
      return res
        .status(400)
        .send('Invalid role. Role must be either "student" or "teacher".');
    }
    ...
    try {
      ...
      connection
        ...
        .then(([userResults]) => {
          ...
          connection
            .promise()
            .query(checkCourseQuery, checkParams)
            .then(([checkResults]) => {
              if (checkResults.length > 0) {
                return res
                  .status(400)
                  .send(`Member is already a member of the course`);
              }
              ...
              connection
                .promise()
                .query(addQuery, [course_id, user.user_id])
                .then(() => {
                  return res
                    .status(200)
                    .send(
            ...
            .catch((err) => {
              ...
              res
                .status(500)
                .send("Failed to check if member exists in course");
            });
        })
        .catch((err) => {
          console.error("Error adding member to course:", err);
  ...
  countCoursesByTerm: (req, res) => {
    const query = `
      ...
      FROM terms t
      LEFT JOIN courses c ON t.term_id = c.term_id
      GROUP BY t.term_id
    `;
    connection.query(query, (err, results) => {
      if (err) {
        console.log("Error executing query:", err);
        return res.status(500).send("Error executing query count courses by term");
      }
      res.status(200).json(results);
    });
  },
  ...
  // Thêm học kỳ mới
  addTerm: (req, res) => {
    const { term_id, term_name } = req.body;
    const query = `INSERT INTO terms (term_id, term_name) VALUES (?, ?)`;
    connection.query(query, [term_id, term_name], (err, results) => {
      if (err) {
        console.log("Error executing query:", err);
        return res.status(500).send("Error executing query add term");
      }
